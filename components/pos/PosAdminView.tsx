...
const formatEuro = (priceCents: number) => (priceCents / 100).toFixed(2);
...
const PosAdminView: React.FC<Props> = ({ onUnauthorized }) => {
  ...
  const startEditArticle = (article: PosArticle) => {
    ...
    const hsl = hexToHsl(article.bg_color || '#D9C188');
  ...
  const saveEditedArticle = async () => {
    if (!canSave || !editingArticleId) return;
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            <div className="text-lg font-black">
              {isEditing ? 'Artikel bearbeiten' : 'Artikel anlegen'}
            </div>
            <div className="text-sm text-black/50 font-semibold">
              {isEditing
                ? `Bearbeite Artikel ID ${editingArticleId}`
                : 'Neuen Artikel für das aktive Projekt erfassen'}
            </div>
          </div>
          ...
          {isEditing && (
            <button
              type="button"
              onClick={resetForm}
              className="px-4 py-3 rounded-lg bg-[#EAEAEA] text-black font-black text-xs uppercase tracking-widest"
            >
              Abbrechen
            </button>
          )}
            ...
            >
              <div className="flex items-center gap-4 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  <div className="font-black text-lg text-black break-words leading-tight">
                    {article.name || `Artikel #${article.id}`}
                  </div>

                  <div className="text-sm text-black/60 break-words mt-1">
                    {getCategoryLabel(article.category)}
                    {article.serving_label ? ` · ${article.serving_label}` : ''}
                    {' · '}
                    {formatEuro(article.price_cents)} €
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  className="px-4 py-2 text-xs font-black rounded-lg bg-[#DCE8FA] text-black"
                >
                  Bearbeiten
                </button>
